...
int main()
{
	...
	FILE* fp = fopen("./test_wr_pool_server.txt", "w+");
...
{	
	...
	rfts::wr_pool<ibv_recv_wr> wrpool(tsas);
	...
	mlock(wrpool.get_flags_addr(), wrpool.get_flags_length());
	...
	munlock(wrpool.get_flags_addr(), wrpool.get_flags_length());
	ibv_dereg_mr(mr);
	for(int i = 0; i < testnum; ++i)
	{
		s=e=0;
